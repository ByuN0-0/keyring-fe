...
export default function Sidebar({ 
  ...
  const handleAddScope = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch('/vault/scopes', {
        method: 'POST',
        body: JSON.stringify({ scope: addingTo, scope_id: newScopeName })
      });
      setNewScopeName('');
      setAddingTo(null);
      onRefreshScopes();
          ...
          <div key={item.id} className="space-y-1">
            <div className="group flex items-center justify-between">
              <button
                ...
                className={`flex flex-1 items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-bold transition-all ${
              ...
              <button 
                onClick={() => setAddingTo(addingTo === item.id ? null : item.id)}
                className="ml-1 rounded-md p-1 text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-colors"
              >
            ...
            {/* Add Scope Input */}
            {addingTo === item.id && (
              <form onSubmit={handleAddScope} className="ml-9 px-1 py-1">
                <input
                  autoFocus
                  type="text"
                  placeholder="새 탭 이름"
            ...
            <div className="ml-9 space-y-1">
              {scopes
                ...
                .map((s) => (
                  ...
                  >
                    <div className={`h-1 w-1 rounded-full ${activeScopeId === s.id ? 'bg-blue-600' : 'bg-slate-300'}`}></div>
                    {s.scope_id}
                  </button>
                ))}
            </div>
